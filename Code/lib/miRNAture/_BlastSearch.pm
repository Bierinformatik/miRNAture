...
sub blast_strategies {
    ...
    if ($number =~ /\d+/){
        if ($number == 10){ #Default blastn
            ...
            $str = "-dust no -soft_masking false -evalue 0.01 -reward 5 -penalty -4 -gapopen 25 -gapextend 10 -word_size 7 -outfmt 6 -out";
        } elsif ($number == 9){ # Hertel parameters: doi:10.3390/life5010905
...
sub infer_data_queries {
    ...
    ### TODO: Consider here create a new file, based on the fasta files provided on the folder
...
sub write_cmsearch_specific_sequence_group {
    ...
    #my $query_file = "$out_path_infernal/../${genome_tag}_$str.$molecule_query.$query_name_spe.tab.db.location.fasta";
    ...
    #my ($Zvalue, $genome_size_bp) = calculate_Z_value($query_file, "Genome"); #Calculate with genome to compare with other TTOs
