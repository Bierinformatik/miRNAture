...
sub process_all_candidates {
    ...
    foreach my $code (sort keys %{$db_codes}){
        if (exists $$acceptedDB{$code}){ #This miRNA was classified as accepted
            #Check if generated an alignment without break the conservation blocks
            ...
            my $validStructure = validate_secondary_structure_alignment($stockholmFile, $shift->current_dir); #Evaluate generated alignment
...
sub process_length {
    ...
    if (length $consensus > length $match){
        ...
        die "Match is larger than Consensus, this is impossible\n";
    }
    return $match;
}
...
sub analysis_stockholm_file {
    ...
    my $matchComplete = $aln->match_line(); #Simbols indicating strong, medium or low conservation
