...
sub cmsearch_mirbase_parallel {
	...
	foreach my $path_cm_specific (@$path_cm){
		...
		system("parallel $cmsearch_path -E 0.015 --notrunc -Z $zscore --nohmmonly --tblout $outFolder/{/.}_$genomeTag.tab -o $outFolder/{/.}_$genomeTag.out $path_cm_specific/{/.}\.cm $genome 1> /dev/null ::: $path_cm_specific/*\.cm");
	}
	return;
}
