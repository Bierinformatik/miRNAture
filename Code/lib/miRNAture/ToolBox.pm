...
use File::Copy; #To copy files from Perl
use Bio::DB::Fasta;
use Bio::Seq;
use Bio::SeqIO;
use RNA;
...
sub get_basic_files {
    ...
    my $data_folder = "$working_folder/Data";
...
=head1 check_format 
    Title: check_format 
    Usage: check_format(nameC, nameC, 1);
    Function: Recognize the file format of input data addressed on configuration file,
          until now only fasta format is allowed. 
    Returns: The type of format in a scalar variable, it saves this information
...
#Detect the format: List, fasta or Stockholm
#	while (<$IO>){
#		chomp;
#		if ($_ =~ m/^>/){
#			$format = "1.fasta";
...
#			last;
#		} else {
#			my $msj="The file format is incorrect, it have to be fasta or Stockholm";
#			error($msj);
#		}			
#}
#	if ($mode == 1){ #Start with config file
#		store_conf_file($format, $name);
#		#system "cp $file Results";
#} elsif ($mode == 2){ #Just iterate, no config file
#		#system "cp $file ../Results";
#		return "$format";
#}
#	close $IO;
...
#sub modify_chr {
#    my ($chr, $SPE) = @_;
#    if($SPE eq "ciin" || $SPE eq "dare"){ #adopted
#        if ($chr =~ m/^HT/){
#            $chr .= ".1";
#        } elsif ($chr =~ m/^Zv9/){
#            ;
...
#        #$chr =~ s/(.*)/adopted\_$1/g;
#    } elsif ($SPE eq "cisa") {
#        $chr =~ s/reftig\_(.*)/R$1/g;
#    } elsif ($SPE eq "pema" || $SPE eq "lach") {
#        #if( exists $genome_map{$_[0]}){
#        #        $_[0] = $genome_map{$_[0]};
#    } elsif ($SPE eq "saco"){
#        $chr .= ".1";
#    } else {
#        ; #print "Not recognized specie!\n";
#    }
#    return $chr;
#}
...
sub cmsearch {
    ...
    if (-e "$path_cm/${nameCMFinal}" && !-z "$path_cm/${nameCMFinal}"){
        #my $param = "--cpu 5 -Z $zscore --noali --nohmmonly --tblout $outFolder/${nameCM}_$genomeTag.tab $path_cm/${nameCMFinal} $genome";
