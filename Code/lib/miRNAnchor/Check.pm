...
sub process_fasta_sequences {
    ...
    my $database_codes = load_database_miRNAs($shift->final_miRNA_database->stringify); #Load all the data from the miRNA table
...
sub check_candidate {
    ...
    foreach my $id (sort keys %{ $shift->fasta_results }){
        ...
        #my $pass3 = validate_secondary_structure_alignment($shift, $shift->output_folder, $id, $shift->db_codes, $shift->accession_number_RFAM, $shift->current_dir);
        #if ($pass3 =~ m/^No_valid|^NA/){
        #    next;	
        #}
...
sub validate_secondary_structure_alignment {
    my ($sto, $current_dir) = @_;
    my $result = system("$current_dir/lib/miRNAnchor/evaluate_conserved_str.py $sto 2>/dev/null 1>/dev/null");
    my $evaluation_result;
    if ($result == 0){ #Success!
        $evaluation_result = `$current_dir/lib/miRNAnchor/evaluate_conserved_str.py $sto 2>&1`;
...
sub discard_candidate {
    ...
    #print $DISCARDED "$id\t$$fasta_candidates{$id}\t$reason\n";
...
sub check_if_valid {
    ...
    if (-e $file && !-z $file){
        ...
        $recall = 0;
        #my $code = $file; 
        #$code =~ s/(\/.*\/)(RF[0-9]+\/)(H.*)(\/Output\/)(\/.*.out\/)(R.*.txt)/$3/g; #H1595453243
        #print_result("The sequence $code failed the mature annotation");
    }
    return $recall;
}
...
#        my $start3p = $all[12];
#        my $end3p = $all[13];
#        $info{$id} = [ $fam, $rfamily, $start5p, $end5p, $start3p, $end3p ];
#    }
#    while (<$POSITIVE>){
#        chomp;
#        my @all = split /\s+|\t/, $_;
...
#        my $idfam = $all[11];
#        my $idrfam = $all[3];
#        $positive{$id} = [$chr, $strand, $start, $end, $idfam, $idrfam, $evalue];
#    }
#    generate_gff($shift->gff_file, \%positive, \%info);
#    generate_bed($shift->bed_file, \%positive, \%info);
#    return;
#}
