...
sub cmsearch_specific_sequence {
	...
	foreach my $cm_models_path_specific (@$cm_models_path){
		...
		if (-e $model && !-z $model){
			my $param = "--cpu 10 -E 0.015 --notrunc -Z $Zscore --nohmmonly --noali --toponly --tblout $out_path_infernal/${genome_tag}.${cm}.tab $model $fasta";
...
sub runNhmmer {
	...
	foreach my $hmms_path_specific (@$path_hmm){
		if (-e "$hmms_path_specific/$HMM.hmm"){
			$param = "--cpu 10 -E 0.015 -Z $zvalue --noali --tblout $out_folder/$species/$species.$HMM.tab $hmms_path_specific/$HMM.hmm $genome";
